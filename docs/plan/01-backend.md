...
**Estado: ✅ Completo — 44 tests, 8 suites, cobertura > 70%**
...
| GET | `/tickets/:key` | Sí | Devuelve un ticket por su **key** (ej. KAN-1) | 200 |
| GET | `/tickets/search?q=` | Sí | Busca tickets por keyword | 200 |
| POST | `/context` | Sí | Recibe `{ ticketIds: string[] }`, devuelve el `.md` como descarga | 200 |
...
- [x] `backend/middlewares/auth.middleware.js` — verifica JWT Bearer, adjunta `req.user`, lanza `AppError` 401 si inválido
...
Usuarios en Map en memoria (sin base de datos).

- [x] `backend/models/user.model.js` — schema Zod: `{ id, email, passwordHash, role: 'admin' | 'user' }`
- [x] `backend/services/auth.service.js` — `register()` con bcrypt + UUID, `login()` con JWT, logs Pino
- [x] `backend/controllers/auth.controller.js` — handlers limpios, sin validación (delegada al middleware)
- [x] `backend/routes/auth.routes.js`:
  ```
  POST /auth/register  →  authLimiter  →  validate(credentialsSchema)  →  auth.controller.register
  POST /auth/login     →  authLimiter  →  validate(credentialsSchema)  →  auth.controller.login
  ```
- [x] `backend/__tests__/auth.service.test.js` — 5 tests
- [x] `backend/__tests__/auth.controller.test.js` — 4 tests
...
- [x] `npm test -- --coverage` — 44 tests, cobertura > 70%
